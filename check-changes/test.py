...
from action import check_changes


class TestCheckChanges(unittest.TestCase):
    def test_simple_glob_matches(self):
        """Test basic glob pattern matches."""
        self.assertTrue(check_changes("*.py", ["test.py"]))
        self.assertTrue(check_changes("*.py", ["src/main.py"]))
        self.assertFalse(check_changes("*.py", ["test.txt"]))
    ...
    def test_complex_glob_matches(self):
        ...
        self.assertTrue(
            check_changes("{.github/*/*.yml,*/action.yml}", ["custom/action.yml"])
        )
        self.assertFalse(
            check_changes("{.github/*/*.yml,*/action.yml}", ["custom/config.yml"])
        )

        # Test .github/workflows/*.yml
        self.assertTrue(
            check_changes(".github/workflows/*.yml", [".github/workflows/deploy.yml"])
        )
        self.assertFalse(
            check_changes(".github/workflows/*.yml", [".github/actions/deploy.yml"])
        )

        # Test *{.md,.yml}
        self.assertTrue(check_changes("*{.md,.yml}", ["README.md"]))
        self.assertTrue(check_changes("*{.md,.yml}", ["config.yml"]))
        self.assertTrue(check_changes("*{.md,.yml}", ["docs/guide.md"]))
        self.assertFalse(check_changes("*{.md,.yml}", ["script.py"]))

    def test_multiple_files(self):
        """Test with multiple changed files."""
        changed_files = ["src/main.py", "README.md", "tests/test_app.js"]

        self.assertTrue(check_changes("*.py", changed_files))
        self.assertTrue(check_changes("*{.md,.yml}", changed_files))
        self.assertFalse(check_changes(".github/workflows/*.yml", changed_files))
        ...
        for pattern in patterns:
            if pattern in test_cases:
                for file_path in test_cases[pattern]["should_match"]:
                    self.assertTrue(
                        check_changes(pattern, [file_path]),
